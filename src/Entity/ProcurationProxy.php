...
class ProcurationProxy implements RecaptchaChallengeInterface
{
    ...
    public function processAvailabilities(): void
    {
        foreach ($this->getProcurationProxyElectionRounds() as $ppElectionRound) {
            $ppElectionRound->setFrenchRequestAvailable($this->hasFreeSlots()
                && self::MAX_FRENCH_REQUESTS > $this->countFrenchRequests($ppElectionRound->getElectionRound()));
        }

        foreach ($this->getProcurationProxyElectionRounds() as $ppElectionRound) {
            $ppElectionRound->setForeignRequestAvailable($this->hasFreeSlots()
                && $this->getForeignRequestsLimit() > $this->countForeignRequests($ppElectionRound->getElectionRound()));
        }
    }

    private function processFrenchAvailability(ProcurationProxyElectionRound $ppElectionRound): void
    {
        dump($ppElectionRound);
        $ppElectionRound->setFrenchRequestAvailable($this->hasFreeSlots()
            && self::MAX_FRENCH_REQUESTS > $this->countFrenchRequests($ppElectionRound->getElectionRound()));
    }
